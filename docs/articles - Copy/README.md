...
| [deployment-api-lambda-notes.md](./deployment-api-lambda-notes.md) | API Lambda deployment: what we did (app.ts, lambda/api.ts, build:api-lambda, externals, zip, Lambda + API Gateway), current blocker (Dynamic require of node:crypto), and next steps to try. |
