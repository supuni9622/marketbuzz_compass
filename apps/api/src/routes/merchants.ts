...
export async function merchantsRoutes(
  ...
  app.get(
    "/refunds",
    {
      ...
      schema: {
        description:
          "Paginated list of refund merchants for a given month. Source: refund_merchants_monthly.",
        tags: ["Merchants"],
        querystring: {
          type: "object",
          properties: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        app.log.error(err);
        return reply.status(500).send({ error: "Failed to fetch refund list" });
      }
    }
  );
  ...
  app.get(
    "/uninstalls",
    {
      ...
      schema: {
        description:
          "Paginated list of uninstall merchants for a given month. Source: uninstall_merchants_monthly.",
        tags: ["Merchants"],
        querystring: {
          type: "object",
          properties: {
  ...
  app.get(
    "/onhold",
    {
      ...
      schema: {
        description:
          "Paginated list of charges in ONHOLD status (payment at risk) for a given month. Source: charges_raw.",
        tags: ["Merchants"],
        querystring: {
          type: "object",
          properties: {
    ...
    async (request, reply) => {
      ...
      try {
        const supabase = db.get();
        const result = await listOnHoldCharges(supabase, {
          month,
          app_id: q.app_id || undefined,
          page: q.page ? parseInt(q.page, 10) : undefined,
          page_size: q.page_size ? parseInt(q.page_size, 10) : undefined,
        ...
        app.log.error(err);
        return reply.status(500).send({ error: "Failed to fetch on-hold charges" });
      }
    }
  );
}
